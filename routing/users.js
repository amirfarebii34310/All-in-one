...
router.get("/profile", isAuthenticated, async (req, res) => {
	...
	let identifikasiMember = 'Powered By Amir';
	res.render('profile', {
...
router.post("/register", async (req, res, next) => {
	try {
		...
		return validasi(req, res, "API AMIR || VERIFICATION", next);
	} catch (err) {
		console.log(err);
...
router.post("/updates", async (req, res) => {
	...
	let bool = ["false", "true"];
	const requests = (status, result) => res.json({ status, creator: "Amir", result });
